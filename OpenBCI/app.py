...
#=======
		...
		self.filter_outputs['dc_offset'][self.currentChannel][:-self.window_size] = self.filter_outputs['dc_offset'][self.currentChannel][self.window_size:]
		self.filter_outputs['dc_offset'][self.currentChannel][-self.window_size:] = dcOutput
		self.plot_buffer['dc_offset'][self.currentChannel][:-self.window_size] = self.plot_buffer['dc_offset'][self.currentChannel][self.window_size:]
		self.plot_buffer['dc_offset'][self.currentChannel][-self.window_size:] = dcOutput
	...
	def process_raw(self,channels=[0],meth='live'):
		...
		for i in self.channels_to_process:
			self.currentChannel = i			
			for _n in range(self.data_buffer.shape[0]):
				# shift window_size bytes from raw_buffer and add new bytes
				self.raw_buffer[i][:-self.window_size] = self.raw_buffer[i][self.window_size:]
				self.raw_buffer[i][-self.window_size:] = self.data_buffer[_n,:,i]
				...
				self.filter_outputs['spec_analyser'][i] = np.append(self.filter_outputs['spec_analyser'][i], self.filter_outputs['bandpass'][i][-self.window_size:])
				if(self.filter_outputs['spec_analyser'][i].reshape(-1).shape[0] == self.window_size*self.spec_analyse):
					self.g[i]= 1
